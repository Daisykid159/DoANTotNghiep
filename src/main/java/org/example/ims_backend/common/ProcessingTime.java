...
public class ProcessingTime {
    ...
    public static Date calculateOverlapAndNewExpire(Date createA, Date expiredA, Date createB, Date expiredB) {
        ...
        if (expiredB.before(expiredA)) {
            ...
            return new Date(expiredA.getTime() + overlapDuration);
        }

        // Trường hợp expiredB lớn hơn hoặc bằng expiredA, cộng thời gian giao
        return new Date(expiredB.getTime() + overlapDuration);
    }
    public static void main(String[] args) throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date createA = sdf.parse("2025-01-01 10:00:00");
        Date expiredA = sdf.parse("2025-01-03 10:00:00");
        Date createB = sdf.parse("2025-01-01 08:00:00");
        Date expiredB = sdf.parse("2025-01-04 08:00:00");

        System.out.println("expiredB mới: " + calculateOverlapAndNewExpire(createA, expiredA, createB, expiredB));
    }
}
